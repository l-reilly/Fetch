...
router.post("/signup",isLoggedOut, (req, res) => {
  ...
  if (!email || !name || !city) {
    return res
      .status(400)
      .render("public/signup", { errorMessage: "Please provide your email." });
  }
  ...
  User.findOne({ email }).then((found) => {
    ...
    return bcrypt
      ...
      .then((user) => {
        ...
        res.redirect("/private/pets/feed");
      })
      .catch((error) => {
...
router.post("/login", isLoggedOut, (req, res, next) => {
  ...
  if (!email) {
    return res
      .status(400)
      .render("public/login", { errorMessage: "Please provide your username." });
  }
...
router.get("/logout", isLoggedIn, (req, res) => {
  req.session.destroy((err) => {
    if (err) {
      return res
        .status(500)
        .render("public/logout", { errorMessage: err.message });
    }
    res.redirect("/");
  });
});
